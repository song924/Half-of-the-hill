<template>
  <div id="wuziqi">
    <div class="Fbuttons">
      ...
      <!-- <input type="button" value="黑棋" @click="selectqz(1)" /> -->
...
export default {
  ...
  mounted() {
    ...
    this.sockets.listener.subscribe('link_get_piece', (data) => {
        ...
        var dx = data.split(",")[0]
        var dy = data.split(",")[1]
        this.pieceColor = data.split(",")[2]
        console.log("接收的参数",dx,dy,this.pieceColor)
        if (this.pieceMapArr[dx / 30 - 1][dy / 30 - 1] == 0) {

            console.log("落下棋子", dx, dy, this.pieceColor);
            
            this.drawPiece(dx, dy, this.pieceColor); //落下棋子

            this.pieceMapArr[dx / 30 - 1][dy / 30 - 1] = this.pieceColor;
    ...
    if (!myCanvas.getContext) {
      ...
      canvas.addEventListener("click", e => {
        ...
        if (this.pieceFlag) {
          ...
          this.toSocket(dx,dy,this.pieceColor)
          ...
          arr01[m][n].onclick = function() {
            ...
            if (!that.flag) {
              if (that.pieceMapArr[m][n] == 0) {
                //黑白交换下棋
                // console.log(this);
                // console.log('落下棋子', that.pieceColor)
                //确保填充颜色正确进行了判断
                if (
                  this.className == "qz" &&
                  that.step % 2 == 0 &&
                  ...
                  //下棋填充黑颜色
                  this.style.backgroundColor = that.pieceColor;
                  //写入棋盘数组
                  that.pieceMapArr[m][n] = that.pieceColor;
                  //历史记录位置
                  that.history.length = that.step;
                  that.history.push({
                    m,
                    n,
                    color: that.pieceColor
                  });
                ...
                //检查当前是否赢了
                for (var i = 0; i < 4; i++) {
                  that.checkWin(m, n, that.pieceColor, that.checkMode[i]);
                }
                that.step++;
